...
/** Get item to view */
$itemID = $_GET['itemID'];
if(!$itemID) header("Location: CustomerPortal.php");
...
function PrintImageCarousel($pictures) {
	...
	for($i = 1; $i < sizeof($pictures); $i++)
		...
		_END;
	$picture = substr($pictures[0]['directory'], 1);
	echo <<<_END
	      </ol>
	...
	for($i = 1, $picture = substr($pictures[$i]['directory'], 1); $i < count($pictures); $i++, $picture = substr($pictures[$i]['directory'], 1))
		echo <<<_END
